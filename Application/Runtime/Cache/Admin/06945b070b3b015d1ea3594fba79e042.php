                    ...
                    <div id="horizontal-form">
                        <form class="form-horizontal" role="form" id="myform">
                            <div class="form-group">
                                ...
                                <div class="col-sm-6">
                                    <input class="form-control" id="name" placeholder="请输入角色名" name="name" value="<?php echo ($cur["role_name"]); ?>" required="required" type="text">
                                </div>
...
<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
    <?php if(is_array($authParent)): $i = 0; $__LIST__ = $authParent;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><div class="panel panel-info">
        ...
        <div id="collapseOne<?php echo ($vo1["auth_id"]); ?>" class="panel-collapse collapse <?php if($i == 1): ?>in<?php endif; ?>" role="tabpanel" aria-labelledby="headingOne">
            <div class="panel-body">
                <input type="checkbox" <?php if(in_array($vo1['auth_id'],$auth_ids_array)): ?>checked="checked"<?php endif; ?> class="checkbox-inline" name="auth[]" value="<?php echo ($vo1["auth_id"]); ?>"><?php echo ($vo1["auth_name"]); ?><br>
                <?php if(is_array($authSon)): $i = 0; $__LIST__ = $authSon;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i; if($vo2['auth_pid'] == $vo1['auth_id']): ?><input type="checkbox" <?php if(in_array($vo2['auth_id'],$auth_ids_array)): ?>checked="checked"<?php endif; ?> name="auth[]" class="checkbox-inline" value="<?php echo ($vo2["auth_id"]); ?>"><?php echo ($vo2["auth_name"]); endif; endforeach; endif; else: echo "" ;endif; ?>
            </div>
        </div>
    </div><?php endforeach; endif; else: echo "" ;endif; ?>
</div>
...
<!-- /Page Body -->
    ...
    <script>
        $("#myform").submit(function(){
            var url = '/team/index.php/Admin/Role/update.html?id=9';
